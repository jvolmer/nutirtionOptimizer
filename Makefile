CXX      = g++
CXXFLAGS = -Wall -Wno-deprecated-declarations -std=c++11
LDFLAGS  = -lm -lglpk
...
TESTOBJECTS = food_test.o store_test.o person_test.o solver_test.o analyzer_integrationtest.o
...
store.test : store_test.o store.o food.o person.o solver.o
	$(CXX) $(CXXFLAGS) $^ -o $@ $(INC) $(LDFLAGSTEST) $(LDFLAGS)
	@echo Running $@
	@./$@ #--log_level=test_suite
	@echo 
...
store_test.o : store_test.cpp store.hpp food.hpp solver.hpp person.hpp
	$(CXX) $(CXXFLAGS) -c $< -o $@ $(INCTEST)
person_test.o : person.hpp
solver_test.o : solver.hpp
analyzer_integrationtest.o : analyzer_integrationtest.cpp analyzer.hpp store.hpp solver.hpp person.hpp food.hpp
	$(CXX) $(CXXFLAGS) -c $< -o $@ $(INCTEST)
